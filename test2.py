...
while (True): 
   ...
   # Capture frame-by-frame 
    ret, frame = cap.read() 
    # frame = cv2.imread('singlemarkersoriginal.png', cv2.IMREAD_COLOR)
    size = frame.shape 
    ...
    if len(res2[0]) > 0: 
        ...
        x1 = (res2[0][0][0][0][0], res2[0][0][0][0][1]) 
        x2 = (res2[0][0][0][1][0], res2[0][0][0][1][1]) 
        x3 = (res2[0][0][0][2][0], res2[0][0][0][2][1]) 
        x4 = (res2[0][0][0][3][0], res2[0][0][0][3][1]) 
    ...
    if len(res2[1]) > 0: # if aruco marker detected
        count = 0 
        for ids in res2[1]:
            ...
            pts_dst = np.array([[res2[0][0][0][0][0], res2[0][0][0][0][1]], [res2[0][0][0][1][0], res2[0][0][0][1][1]], [res2[0][0][0][2][0], res2[0][0][0][2][1]], [res2[0][0][0][3][0], res2[0][0][0][3][1]]]) 
            pts_src = pts_dst 
            h, status = cv2.findHomography(pts_src, pts_dst) 
      
            imgWithAruco = cv2.warpPerspective(im_src, h, (im_dst.shape[1], im_dst.shape[0])) 
      
            rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(res2[0], markerLength, camera_matrix, dist_coeffs) 
      
            imgWithAruco = cv2.aruco.drawDetectedMarkers(imgWithAruco, res2[0], res2[1])
            imgWithAruco = cv2.aruco.drawAxis(imgWithAruco, camera_matrix, dist_coeffs, rvec[count], tvec[count], 1)
            cameraPose = cameraPoseFromHomography(h)
            count = count+1 
          
  
    cv2.imshow("aruco", imgWithAruco)   # display 
    # cv2.imwrite('frame.jpg',imgWithAruco)
